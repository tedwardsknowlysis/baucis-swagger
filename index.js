...
Swagger.prototype.finalize = function(app, path) {
	var controllers = this.__controllers;
	var baseApiPath = path ? path : '/api';
	...
	controllers.forEach(function(controller) {
		...
		app.use(baseApiPath + '/api-docs/' + route, function (request, response, next) {
	...
	app.use(baseApiPath + '/api-docs', function (request, response, next) {
		response.set('X-Powered-By', 'Baucis');
